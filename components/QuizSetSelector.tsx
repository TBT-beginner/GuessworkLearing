    ...
    <div className="w-full mx-auto pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-8">
        {sets.map((set) => {
          ...
          const isLocked = attemptsUsed >= MAX_ATTEMPTS && !isPassed;

          return (
            <div 
              key={set.id} 
              className={`
                group relative bg-white rounded-3xl p-8 border-2 transition-all duration-300
                ${isLocked ? 'border-slate-200 opacity-75 bg-slate-50' : 'border-slate-200 hover:border-indigo-300 hover:shadow-xl cursor-pointer'}
              `}
              onClick={() => !isLocked && onSelect(set)}
            >
              <div className="flex justify-between items-start mb-6">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${isPassed ? 'bg-emerald-100 text-emerald-600' : 'bg-indigo-50 text-indigo-600'}`}>
                  {isPassed ? <CheckCircle2 className="w-8 h-8" /> : <BookOpen className="w-8 h-8" />}
                </div>
                <div className="flex flex-col items-end">
                    <span className={`text-sm font-bold px-3 py-1.5 rounded-full ${
                        isPassed ? 'bg-emerald-100 text-emerald-700' :
                        isLocked ? 'bg-rose-100 text-rose-700' :
                        'bg-slate-100 text-slate-600'
                    }`}>
                        {isPassed ? 'COMPLETED' : isLocked ? 'LOCKED' : 'AVAILABLE'}
                    </span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-base pt-6 border-t border-slate-100">
                ...
                <span className="text-indigo-600 font-bold text-lg flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
